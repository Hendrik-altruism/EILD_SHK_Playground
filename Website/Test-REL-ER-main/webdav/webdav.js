 ...
 */
const webDavLink =
  "https://lea.hochschule-bonn-rhein-sieg.de/webdav.php/db_040811/ref_1375661";
...
async function insertRun(file, dateKey) {
  if (file.results[file.results.length - 2].hasOwnProperty("correct")) {
    ...
    const phrase = toKey(file.phrases[file.results.length - 2].key);
    const correct = file.results[file.results.length - 2].correct ? 0 : 1;
...
async function startTrainer(phrase) {
  setTimeout(async () => {
    ccm.start(".../ccm.er_rel_trainer.js", {
      ...
      html: [
        "ccm.load",
        { url: "./webdav/webdav_templates.js", type: "module" },
      ],
