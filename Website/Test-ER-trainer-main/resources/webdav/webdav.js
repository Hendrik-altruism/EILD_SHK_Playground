 ...
 */
const webDavLink =
  "https://lea.hochschule-bonn-rhein-sieg.de/webdav.php/db_040811/ref_1457822";
...
let filter1 = ""; //Filtereinstellung für die Suchfunktion
let filter2 = ""; //Filtereinstellung für die Suchfunktion
let filter3 = ""; //Filtereinstellung für die Suchfunktion
...
(async function createPopup() {
  ...
  <div class="modal show" id="myModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style="display: block;overflow: auto">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <label for="exampleInputEmail1">Studenten Kürzel</label>
  ...
  `;
  document.getElementById("er_trainer-1").insertAdjacentHTML("afterend", m);
})();
...
async function userDataPlot() {
  ...
  let dataPop = `
    <div class="modal-dialog" role="document" style="min-width:500px;max-width:1000px;margin-bottom:10rem">
      <div class="modal-content">
        <div class="modal-header">
  ...
  setTimeout(
    () =>
      window.addEventListener("click", ({ target }) => {
        const pop = target.closest(".modal-dialog");
...
async function login() {
  ...
  let StudPW = document.getElementById("StudPw");
  if (StudData.value == "" || StudPW.value.toString() == "") {
    document.getElementById("errorMessage").style.display = "block";
    return false;
  }
  tag = hash(StudData.value + StudPW.value.toString());
  try {
    if (webDAVenabled) initCon(StudData, StudPW);
    call();
  } catch (err) {
    document.getElementById("errorMessage").style.display = "block";
    StudPW.value = "";
...
async function insertRun(file, dateKey) {
  if (file.sections[file.sections.length - 1].hasOwnProperty("correct")) {
    let data = await getContents(tag);

    const phrase = toKey(file.phrases[file.sections.length - 1].key);
    const correct = file.sections[file.sections.length - 1].correct ? 0 : 1;
      ...
      setTimeout(
        () =>
          myModal.addEventListener("click", ({ target }) => {
            const pop = target.closest("#dropdownbtn");
            const clickedOnClose = pop == null;
            if (clickedOnClose) {
              try {
              ...
              <tr>
                  <th>
                    <div class="search-wrapper">
                      <label for="searchIndex">Phrase</label>
                      <input type="number" id="searchIndex" class="search" data-search>
                    </div>
                  </th>
                  <th>
                    <div class="search-wrapper">
                      <label for="searchResult">Ergebniss</label>
                      <input type="search" id="searchResult" class="search" data-search>
                    </div>
                  </th>
                  <th>
                    <div class="search-wrapper">
                      <label for="searchDate">Datum</label>
                      <input type="search" id="searchDate" class="search" data-search>
                    </div>
                  </th>
              </tr>
          </thead>
...
async function showContent(outputSpan) {
  ...
  for (const [key, value] of Object.entries(studData)) {
    ...
    for (let j = 0; j < aktRuns.length; j++) {
      ...
      if (
        decript(key).toString().includes(filter1) &&
        "true".includes(filter2) &&
        newDate.includes(filter3)
      ) {
...
function decript(skey) {
  ...
  for (let i = 0; i < phrases.length; i++) {
    if (
      toKey(phrases[i].entities.toString()) === toKey(ent.toString()) &&
...
function callPopup(phraseID) {
  ...
  p +=
    ...
    phrase.solution.toString() +
    "</span></div>";
  p +=
    '<div class="DataField"><span class="identifier">Kommentare: </span><span class="variable">' +
    phrase.comments.toString() +
    "</span></div>";
  p +=
...
async function startTrainer(phrase) {
  setTimeout(async () => {
    ccm.start(".../ccm.er_trainer.js", {
      root: document.querySelector(".modal-body"),
      phrases: [phrase],
      html: [
        "ccm.load",
        { url: "./resources/webdav/webdav_templates.js", type: "module" },
      ],
      onfinish: async (event, instance) => {
        ...
        if (result.sections[0].hasOwnProperty("correct")) {
          let data = await getContents(tag);
          data[toKey(phrase.key)][result.correct ? 0 : 1].push(dateKey);
          try {
            await setContents(tag, data);
          } catch (error) {}
        }
...
async function searchListener() {
  const input1 = document.querySelector("#searchIndex");

  input1.addEventListener("input", (e) => {
    filter1 = e.target.value.toLowerCase();
    showContent(0);
  });

  const input2 = document.querySelector("#searchResult");

  input2.addEventListener("input", (e) => {
    filter2 = e.target.value.toLowerCase();
    showContent(0);
  });

  const input3 = document.querySelector("#searchDate");

  input3.addEventListener("input", (e) => {
    filter3 = e.target.value.toLowerCase();
    showContent(0);
  });
}
...
function getKeys() {
  ...
  phrases.forEach((element) => {
    result.push(toKey(element.entities.concat(element.solution).join("_")));
  });
  return result;
}
...
let testData = {
  ...
  Namensschild_Teilnehmer_1_c: [
    ["2023-1-2", "2023-3-7", "2023-3-11"],
    ["2023-2-23", "2023-2-11", "2023-3-7", "2023-3-11", "2023-3-11"],
  ],
  Anhnger_Fahrzeug_1_c: [
    ["2023-1-11", "2023-3-11"],
    ["2023-2-2", "2023-3-6", "2023-3-7", "2023-3-11"],
  ],
  Bahnhof_Stadt_1_cn: [
  ...
  Verein_Kontaktperson_1_n: [
    ["2023-4-4", "2023-4-17", "2023-4-17"],
    ["2023-2-23", "2023-2-11", "2023-3-7", "2023-4-5", "2023-4-17"],
  ],
  Musikstck_Komponist_1_n: [
    ["2023-1-11", "2023-3-11"],
    ["2023-2-2", "2023-3-6", "2023-3-7", "2023-3-11"],
  ],
  Topf_Deckel_c_1: [
  ...
  XChromosom_YChromosom_c_c: [
    ["2023-4-4", "2023-4-17", "2023-4-17"],
    ["2023-2-23", "2023-2-11", "2023-3-7", "2023-4-5", "2023-4-17"],
  ],
  Weibchen_Mnnchen_c_c: [
    ["2022-1-2", "2023-3-7", "2023-3-11"],
    ["2022-1-11", "2023-3-11"],
  ],
  Weinflasche_Weinsorte_c_cn: [
    ["2023-2-23", "2023-2-11", "2023-3-7", "2023-3-11", "2023-3-11"],
    ["2023-2-2", "2023-3-6", "2023-3-7", "2023-3-11"],
  ],
  Insekt_Flgel_c_cn: [
  ...
  Inventar_Gegenstand_cn_c: [
    ["2023-4-4", "2023-4-17", "2023-4-17"],
    ["2023-2-23", "2023-2-11", "2023-3-7", "2023-4-5", "2023-4-17"],
  ],
  Mitarbeiter_Aufgabe_cn_c: [
    ["2023-4-4", "2023-4-17", "2023-4-17"],
    ["2023-2-23", "2023-2-11", "2023-3-7", "2023-4-5", "2023-4-17"],
  ],
  Kunde_Produkt_cn_cn: [
  ...
  Teilnehmer_Bescheinigung_cn_cn: [
    ["2023-4-4", "2023-4-17", "2023-4-17"],
    ["2023-2-23", "2023-2-11", "2023-3-7", "2023-4-5", "2023-4-17"],
  ],
  Sprache_Dozent_cn_n: [
  ...
  Haltestelle_Haltestelle_cn_cn: [
    ["2022-1-2", "2023-3-7", "2023-3-11"],
    ["2022-1-11", "2023-3-11"],
  ],
  Student_Professor_Lehrveranstaltung_cn_cn_cn: [
    ["2023-1-11", "2023-3-11"],
    ["2023-2-2", "2023-3-6", "2023-3-7", "2023-3-11"],
  ],
  Pilot_Flugzeug_Flugroute_cn_cn_n: [
    ["2023-4-4", "2023-4-17", "2023-4-17"],
    ["2023-2-23", "2023-2-11", "2023-3-7", "2023-4-5", "2023-4-17"],
  ],
  Programmierer_Quelltext_Version_cn_n_1: [
    ["2023-2-23", "2023-2-11", "2023-3-7", "2023-3-11", "2023-3-11"],
    ["2023-2-2", "2023-3-6", "2023-3-7", "2023-3-11"],
  ],
  ...
  Angeklagter_Richter_Staatsanwalt_Verteidiger_n_cn_cn_cn: [
    ["2023-4-4", "2023-4-17", "2023-4-17"],
    ["2023-2-23", "2023-2-11", "2023-3-7", "2023-4-5", "2023-4-17"],
  ],
  Schauspieler_Regisseur_Produzent_Buchautor_cn_cn_cn_cn: [
    ["2022-1-2", "2023-3-7", "2023-3-11"],
    ["2022-1-11", "2023-3-11"],
  ],
  Haustier_Hund_Katze_p_d: [
    ["2023-2-23", "2023-2-11", "2023-3-7", "2023-3-11", "2023-3-11"],
    ["2023-2-2", "2023-3-6", "2023-3-7", "2023-3-11"],
  ],
  Paketdienstleister_DHL_Hermes_DPD_p_d: [
    ["2023-1-11", "2023-3-11"],
    ["2023-2-2", "2023-3-6", "2023-3-7", "2023-3-11"],
  ],
  Besucher_Grnder_Sponsor_p_n: [
    ["2023-4-4", "2023-4-17", "2023-4-17"],
    ["2023-2-23", "2023-2-11", "2023-3-7", "2023-4-5", "2023-4-17"],
  ],
  Hund_Schferhund_Mops_Dackel_p_n: [
    ["2023-4-4", "2023-4-17", "2023-4-17"],
    ["2023-2-23", "2023-2-11", "2023-3-7", "2023-4-5", "2023-4-17"],
  ],
  Elternteil_Mutter_Vater_t_d: [
    ["2023-2-23", "2023-2-11", "2023-3-7", "2023-3-11", "2023-3-11"],
    ["2023-2-2", "2023-3-6", "2023-3-7", "2023-3-11"],
  ],
  Adressbuch_Privat_Arbeit_Mobil_t_d: [
    ["2022-1-2", "2023-3-7", "2023-3-11"],
    ["2022-1-11", "2023-3-11"],
  ],
  Hochschulangehriger_Student_Mitarbeiter_t_n: [
    ["2023-4-4", "2023-4-17", "2023-4-17"],
    ["2023-2-23", "2023-2-11", "2023-3-7", "2023-4-5", "2023-4-17"],
  ],
  Person_Besucher_Patient_Personal_t_n: [
    ["2023-1-11", "2023-3-11"],
    ["2023-2-2", "2023-3-6", "2023-3-7", "2023-3-11"],
  ],
};
